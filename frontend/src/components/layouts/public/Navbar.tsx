  ...
  useEffect(() => {
    const checkFooterOverlap = () => {
      ...
      if (isMobile) {
        ...
        if (footerRect.top < window.innerHeight - 16) {
           if (footerRect.top < navBottom) {
             setNavY(-(navBottom - footerRect.top));
           } else {
             setNavY(0);
           }
        }
  ...
  return (
    <>
      ...
      <motion.nav
        ...
        className="fixed bottom-4 lg:top-4 lg:bottom-auto left-1/2 z-[51] w-[95%] lg:max-w-fit"
      >
        <div
          className="bg-white/90 dark:bg-[#1e1e1e]/90 backdrop-blur-md border border-black/20 dark:border-white/10 rounded-2xl lg:rounded-full px-2 py-2 lg:px-6 lg:py-2 shadow-[0_4px_20px_rgba(0,0,0,0.05)] flex items-center transition-all duration-300 hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] dark:hover:shadow-[0_8px_30px_rgba(255,255,255,0.05)] overflow-hidden"
        >
          <div
            title="Gilang Abdian"
            ...
            {menus.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  title={item.title}
                  className={`group flex-shrink-0 flex flex-col lg:flex-row items-center justify-center gap-1 lg:gap-1.5 p-2 lg:px-3 lg:py-1.5 rounded-xl lg:rounded-full text-[10px] lg:text-sm font-bold transition-all duration-200 border whitespace-nowrap active:scale-95 ${
                    isActive
                      ? 'bg-black text-white dark:bg-[#ffffff] dark:text-black shadow-md lg:shadow-none border-transparent'
                      : 'border-transparent hover:border-black/20 dark:hover:border-white/20 text-gray-700 dark:text-gray-300 dark:hover:text-white'
                  }`}
                >
                  <Icon
                    icon={item.icon}
                    className={`w-5 h-5 lg:w-4 lg:h-4 transition-transform group-hover:scale-110 ${isActive && mounted && !isDark ? 'text-white' : ''}`}
                  />
                  <span className="uppercase tracking-tighter lg:tracking-normal lg:capitalize">
                    {item.name}
                  </span>
                </Link>
              );
          ...
          <div className="border-l border-black/20 dark:border-white/20 pl-2 ml-1 lg:pl-4 lg:ml-2 flex-shrink-0">
            ...
            >
              {mounted && (
                <Icon
                  icon={isDark ? 'lucide:sun' : 'si:moon-line'}
